...
const generateAuth = async (
  ...
  const authProviders: string[] = Array.from(
    new Set(
      clusters.map(
...
export const useKubernetesObjects = (
  ...
  const getObjects = useCallback(async (): Promise<ObjectsByEntityResponse> => {
    const auth = await generateAuth(entity, kubernetesApi, kubernetesAuthProvidersApi);
  ...
  const { value, loading, error, retry } = useAsyncRetry(() => getObjects(), [getObjects]);
